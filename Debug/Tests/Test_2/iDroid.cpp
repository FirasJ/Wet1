...
iDroid::iDroid() :
		_versions(), _appsByIDtree(), _appsByDLtree(), _max() {
}

StatusType iDroid::AddVersion(int versionCode) {
	if (versionCode <= 0)
		return INVALID_INPUT;
	if (!_versions.empty() && versionCode <= _versions.getTail().getId())
		return FAILURE;
	_versions.insert(Version(versionCode));
	return SUCCESS;
}

StatusType iDroid::AddApplication(int appID, int versionCode,
		int downloadCount) {
	if (appID <= 0 || versionCode <= 0 || downloadCount < 0)
		return INVALID_INPUT;
	try {
		bool found = false;
		for (List<Version>::Iterator it = _versions.begin();
				it != _versions.end(); ++it) {
...
StatusType iDroid::IncreaseDownloads(int appID, int downloadIncrease) {
	...
	try {
		...
		status = AddApplication(appID, appByID._versionCode,
				appByID._downloads);
		if (status != SUCCESS)
			return status;
	} catch (Tree<DataByID>::TreeIsEmpty& e) {
...
StatusType iDroid::GetAllAppsByDownloads(int versionCode, int** apps,
		int* numOfApps) {
...
class TreeToArray {
	...
	int index;
public:
	TreeToArray(int** apps, int* numOfApps) :
			apps(apps), numOfApps(numOfApps), index(*numOfApps - 1) {
	}
...
StatusType iDroid::getAllApps(const Tree<DataByDowns>& tree, int** apps,
		int* numOfApps) {
...
void mergeLists(const List<DataByDowns>& list1, const List<DataByDowns>& list2,
		List<DataByDowns>& newList) {
...
class FillTree {
public:
	FillTree(const List<DataByDowns>& list) :
			_current(list.begin()) {
	}
...
class UpdateDL {
public:
	UpdateDL(int groupBase, int multiplyFactor) :
			_groupBase(groupBase), _multiplyFactor(multiplyFactor) {
	}
...
class PredicateTree {
public:
	PredicateTree(List<DataByDowns>* belongs, List<DataByDowns>* doesntBelong,
			int groupBase, int multiplyFactor) :
			_belongs(belongs), _doesntBelong(doesntBelong), _groupBase(
					groupBase), _multiplyFactor(multiplyFactor) {
	}
	void operator()(DataByDowns& data) {
		if (data._appID % _groupBase == 0) {
			data._downloads *= _multiplyFactor;
